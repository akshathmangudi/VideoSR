...
for folder_i in range(folder_n):
    ...
    for i in range(cutfr, image_no-cutfr):
        ...
        msg = "frame %d, tar %s, out %s, " % (
            i, str(target_img.shape), str(output_img.shape))
        # target is not dividable by 4
        if (target_img.shape[0] < output_img.shape[0]) or (target_img.shape[1] < output_img.shape[1]):
    ...
    pd.DataFrame(pd_dict).to_csv(os.path.join(
        FLAGS.output, "metrics.csv"), mode=mode)

for num_data in keys:
    sum_dict["FrameAvg_"+num_data] = pd.Series(
        [sum_dict["FrameAvg_"+num_data] / len_dict[num_data]])
    folder_dict["FolderAvg_" +
                num_data] = pd.Series([folder_dict["FolderAvg_"+num_data] / folder_n])
    avg_dict["Avg_" +
             num_data] = pd.Series(np.float32(avg_dict["Avg_"+num_data]))
...
pd.DataFrame(avg_dict).to_csv(os.path.join(
    FLAGS.output, "metrics.csv"), mode='a')
pd.DataFrame(folder_dict).to_csv(
    os.path.join(FLAGS.output, "metrics.csv"), mode='a')
pd.DataFrame(sum_dict).to_csv(os.path.join(
    FLAGS.output, "metrics.csv"), mode='a')
print("Finished.")
