...
import numpy as np
from base import BaseSequenceGenerator

_logger = logging.getLogger("VSR.SOF")
_logger.info(
    "LICENSE: SOF-VSR is implemented by https://github.com/LongguangWang/SOF-VSR.")
...
def optical_flow_warp(image, image_optical_flow):
    ...
    flow_0 = torch.unsqueeze(
        image_optical_flow[:, 0, :, :] * 31 / (w - 1), dim=1)
    flow_1 = torch.unsqueeze(
        image_optical_flow[:, 1, :, :] * 31 / (h - 1), dim=1)
...
class RDB(nn.Module):
    def __init__(self, nDenselayer, channels, growth):
        ...
        self.conv_1x1 = nn.Conv2d(
            channels_buffer, channels, kernel_size=1, padding=0, bias=False)
...
class OFRnet(nn.Module):
    def __init__(self, upscale_factor, is_training):
        ...
        self.final_upsample = nn.Upsample(
            scale_factor=upscale_factor, mode='bilinear')
        ...
        self.bottleneck_L3 = nn.Conv2d(
            64, 2 * upscale_factor ** 2, 3, 1, 1, bias=False)
        self.conv_L3_2 = nn.Conv2d(
            2 * upscale_factor ** 2, 2 * upscale_factor ** 2, 3, 1, 1, bias=True)
    ...
    def forward(self, x):
        ...
        if self.is_training is True:
            x_L1_res = optical_flow_warp(torch.unsqueeze(x_L1[:, 0, :, :], dim=1), optical_flow_L1) - torch.unsqueeze(
                x_L1[:, 1, :, :], dim=1)
        # Level 2
        x_L2 = optical_flow_warp(torch.unsqueeze(
            x[:, 0, :, :], dim=1), optical_flow_L1_upscaled)
        ...
        if self.is_training is True:
            x_L2_res = optical_flow_warp(torch.unsqueeze(x_L2[:, 0, :, :], dim=1), optical_flow_L2) - torch.unsqueeze(
                x_L2[:, 1, :, :], dim=1)
        # Level 3
        x_L3 = optical_flow_warp(torch.unsqueeze(
            x[:, 0, :, :], dim=1), optical_flow_L2)
        ...
        optical_flow_L3 = self.shuffle(
            optical_flow_L3) + self.final_upsample(optical_flow_L2)  # *4
...
class SRnet(nn.Module):
    def __init__(self, upscale_factor, is_training):
        ...
        self.bottleneck = nn.Conv2d(
            384, upscale_factor ** 2, 1, 1, 0, bias=False)
        self.conv_2 = nn.Conv2d(upscale_factor ** 2,
    ...
    def forward(self, x):
        ...
        output = torch.cat((buffer_1, buffer_2, buffer_3,
                           buffer_4, buffer_5, input), 1)
...
class SOFNet(BaseSequenceGenerator):
    ...
    def forward(self, lr_seq):
        x = lr_seq
        input_01 = torch.cat((torch.unsqueeze(
            x[:, 0, :, :], dim=1), torch.unsqueeze(x[:, 1, :, :], dim=1)), 1)
        input_21 = torch.cat((torch.unsqueeze(
            x[:, 2, :, :], dim=1), torch.unsqueeze(x[:, 1, :, :], dim=1)), 1)
        if self.is_training is False:
            flow_01_L3 = self.OFRnet(input_01)
            flow_21_L3 = self.OFRnet(input_21)
        if self.is_training is True:
            res_01_L1, res_01_L2, flow_01_L1, flow_01_L2, flow_01_L3 = self.OFRnet(
                input_01)
            res_21_L1, res_21_L2, flow_21_L1, flow_21_L2, flow_21_L3 = self.OFRnet(
                input_21)
        draft_cube = x
